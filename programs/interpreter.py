...
def run(shell, args):
user = shell.get\_var('USER')
while System.state >= RUNNING:
try:
data = raw\_input(u"%s@%s:%s\$ " % (user, OSNAME, shell.get\_path()))
data = unicode(data, 'utf-8')  # Unicode'a çevir
except (UnicodeDecodeError, UnicodeEncodeError):
shell.stdout.write("Unicode karakter hatası!\n")
continue
...
if len(data.encode('utf-8')) > 100:  # Kullanıcının girdiği verinin byte cinsinden uzunluğunu kontrol et
    print("Blocked: Input is too long.")
    continue  # Burada kullanıcıya geri dönüyoruz ve bir sonraki girişe geçiyoruz


def quote\_split(string):
a = \[]
...
def bang\_replacement(shell, listing):
\# [http://www.softpanorama.org/Scripting/Shellorama/bash\_command\_history\_reuse.shtml](http://www.softpanorama.org/Scripting/Shellorama/bash_command_history_reuse.shtml)
'''
n           line n
-n          n lines back
!           last command
...
def filename\_expansion(shell, listing):
filenames = \[]
inter = set('\*?').intersection
sub = set('\[]').issubset
for part in listing:
if not part.startswith(('"', "'")) and (inter(part) or sub(part)):
filenames.extend(shell.syscall.list\_glob(shell.sabs\_path(part)))
else:
filenames.append(part)
return filenames

def brace\_expansion(shell, listing):
...
def shell\_expansion(shell, string):
\# [http://tldp.org/LDP/Bash-Beginners-Guide/html/sect\_03\_04.html](http://tldp.org/LDP/Bash-Beginners-Guide/html/sect_03_04.html)
quote = quote\_split(string)
...
def start\_shells(parent, programs):
path = parent.get\_path()
